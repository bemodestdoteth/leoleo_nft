...
# 앞머리/뒷머리 + 얼굴과 몸은 같은 색깔로 맞추기 위해 서로 같은 랜덤값으로 결정된다.
def generate_image():
    while(True):
        new_image = {}
...
# Classify traits, 나중에 보지 좋게 수정할 필요가 있다.
...
# 앞머리/뒷머리 + 얼굴과 몸은 같은 색깔로 맞추기 위해 같은 가중치를 공유한다.
total_images = 5 # 123
all_images = []
...
for item in all_images:
    ...
    rgb_im.save('./leoleo_image/' + file_name)

# Dump metadata
local_metadata = "./leoleo/"
with open(local_metadata + "leoleo.json", 'w') as file:
...
f = open(local_metadata + "leoleo.json") 
data = json.load(f)
